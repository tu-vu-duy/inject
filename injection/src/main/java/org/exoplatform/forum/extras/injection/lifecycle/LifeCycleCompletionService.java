 ...
 */
package org.exoplatform.forum.extras.injection.lifecycle;
...
public class LifeCycleCompletionService {
  ...
  private Executor                     executorNotConf;

  private ExecutorCompletionService<?> ecs;

  private ExecutorCompletionService<?> ecsNotConf;
  ...
  public LifeCycleCompletionService(InitParams params) {
    ...
    this.executorNotConf = Executors.newFixedThreadPool(this.configThreadNumber);
    this.ecsNotConf = new ExecutorCompletionService(executorNotConf);
  ...
  public Future<?> addTaskNotConf(Callable callable) {
    return ecsNotConf.submit(callable);
  }
